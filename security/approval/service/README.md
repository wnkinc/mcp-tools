...
The card platform is selected by `APPROVAL_PROVIDER`: `slack` (default) or
`discord`; `telegram` is planned. An unimplemented provider fails closed:
startup refuses it, and `/gate` reports every approval undeliverable.
...
its buttons, the gate can approve itself: e.g. never use the (future) telegram
provider on the account the telegram tool runs as. Discord/Slack button clicks
only arrive via the signature-verified webhooks, which no tool can forge.
...
Setup: `cp env.example .env` and follow its Slack-app or Discord-app steps —
including the one-time webhook URL step (Slack's Interactivity Request URL or
Discord's Interactions Endpoint URL, at
`https://approval.<MCP_DOMAIN>/{slack|discord}/interact`; Discord validates
the endpoint on save, so the sidecar must be live first).
