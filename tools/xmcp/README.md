...
- **Delta from upstream**: `serve()` wiring (Google OAuth, out-of-band approval, guardrail), write-guard + fail-closed allowlist default, MCP `ToolAnnotations` (`readOnlyHint` drives Claude's read-only vs write/delete permission categories; upstream sets none), guardrail outputSchema strip, and approval exemptions auto-derived from the read surface.
